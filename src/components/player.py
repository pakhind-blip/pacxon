...
_DIRECTION_ANGLE = {RIGHT: 0, DOWN: math.pi / 2, LEFT: math.pi, UP: -math.pi / 2}
...
class Player(GameObject, Collision):
    def __init__(self, width=20, height=20, block_size=20, move_delay=4):
        ...
        self.move_delay = move_delay; self.move_timer = 0
        self.lerp_speed = 0.4; self.lives = 3
        self.is_trailing = False
        self.is_iframe = False; self.iframe_duration = 210; self.iframe_timer = 0
        self.blink_speed = 10; self.sword_immune = False
        self._tick = 0
    ...
    def set_position(self, x, y):
        ...
        self.x = self.target_x = x * self.block_size
        self.y = self.target_y = y * self.block_size

    def reset_movement(self):
        self.is_trailing = False; self.direction = RIGHT; self.move_timer = 0

    def update(self): pass   # required by ABC

    # ── movement ───────────────────────────────────────────────────────────
    def move_with_collision(self, keys, grid_manager, item_pos=None):
        if self.is_iframe:
            if grid_manager.get_cell(self.grid_x, self.grid_y) == 1:
                self.iframe_timer -= 1
        ...
        if self.move_timer > 0:
            ...
            # Block reversing while trailing
            if self.is_trailing:
            ...
            if self.is_trailing and self.stats and self.direction != prev_dir:
            ...
            if dx or dy:
                ...
                if 0 <= ngx < grid_manager.width and 0 <= ngy < grid_manager.height:
                    ...
                    if self.is_iframe and next_cell == 0:
                        self.is_iframe = False; self.iframe_timer = 0
                    if not self.is_trailing and next_cell == 0 and self.stats:
                        self.stats.on_trail_start(self.direction)
                    self.grid_x, self.grid_y = ngx, ngy
                    self.target_x = ngx * self.block_size
                    self.target_y = ngy * self.block_size
    ...
    def clamp_to_bounds(self, sw, sh):
        self.x = max(0.0, min(self.x, sw - self.width))
        self.y = max(0.0, min(self.y, sh - self.height))
    ...
    def draw(self, surface, offset_y=0):
        self._tick += 1
        if self.is_iframe and (self.iframe_timer // self.blink_speed) % 2 != 0:
            return
        bs = self.block_size
        cx = int(self.x + bs // 2)
        cy = int(self.y + bs // 2 + offset_y)
        r  = bs // 2 - 1
        angle  = _DIRECTION_ANGLE.get(self.direction, 0)
